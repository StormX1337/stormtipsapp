...
export async function adminOpsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/polls', { preHandler: [app.requireCapability('polls:write')] }, async (request) => {
    ...
    return paginate(polls.map((poll) => serializePoll(poll)), total, query);
  });

  app.post('/polls', { preHandler: [app.requireCapability('polls:write')] }, async (request, reply) => {
    ...
    await audit(request, { action: 'poll.created', entityType: 'Poll', entityId: poll.id, after: input });
    reply.status(201);
    return serializePoll(poll);
  });

  app.patch('/polls/:id', { preHandler: [app.requireCapability('polls:write')] }, async (request) => {
    ...
    await audit(request, { action: 'poll.updated', entityType: 'Poll', entityId: id, after: input });
    return serializePoll(poll);
  });

  app.delete('/polls/:id', { preHandler: [app.requireCapability('polls:write')] }, async (request) => {
    const { id } = parseParams(request, idParamSchema);
  ...
  app.post('/providers', { preHandler: [app.requireCapability('providers:write')] }, async (request, reply) => {
    ...
    const provider = await prisma.apiProvider.upsert({
      ...
      update: {
        ...
        ...(input.apiKey ? { apiKeyEncrypted: encryptSecret(input.apiKey, env.ENCRYPTION_KEY) } : {}),
        isActive: input.isActive,
        priority: input.priority,
        pollIntervalSeconds: input.pollIntervalSeconds,
        rateLimitPerMinute: input.rateLimitPerMinute,
  ...
  app.post('/providers/:slug/test', { preHandler: [app.requireCapability('providers:write')] }, async (request) => {
  ...
  app.post('/providers/sync/:kind', { preHandler: [app.requireCapability('providers:write')] }, async (request, reply) => {
    ...
    await audit(request, { action: 'provider.sync_requested', entityType: 'ApiProvider', after: { kind } });
    reply.status(202);
  ...
  app.put('/settings', { preHandler: [app.requireCapability('settings:write')] }, async (request) => {
    ...
    await audit(request, { action: 'setting.updated', entityType: 'AppSetting', entityId: input.key, after: input });
    return setting;
  });

  app.get('/referral-program', { preHandler: [app.requireCapability('settings:write')] }, async () =>
    referrals.program(),
  );

  app.put('/referral-program', { preHandler: [app.requireCapability('settings:write')] }, async (request) => {
    const input = parseBody(request, referralProgramSchema);
    const saved = await referrals.saveProgram(input);
    await audit(request, { action: 'referral_program.updated', entityType: 'AppSetting', after: input });
    return saved;
  });
  ...
  app.post('/statistics/recompute', { preHandler: [app.requireCapability('statistics:read')] }, async (request, reply) => {

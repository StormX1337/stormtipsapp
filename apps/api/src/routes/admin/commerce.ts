...
export async function adminCommerceRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/plans', { preHandler: [app.requireCapability('plans:write')] }, async (request, reply) => {
    ...
    await audit(request, { action: 'plan.created', entityType: 'SubscriptionPlan', entityId: plan.id, after: input });
    reply.status(201);
    return serializePlan(plan);
  });

  app.patch('/plans/:id', { preHandler: [app.requireCapability('plans:write')] }, async (request) => {
    const { id } = parseParams(request, idParamSchema);
    const input = parseBody(request, upsertPlanSchema.partial());
    const before = assertFound(await prisma.subscriptionPlan.findUnique({ where: { id } }), 'Plan');
  ...
  app.delete('/plans/:id', { preHandler: [app.requireCapability('plans:write')] }, async (request) => {
    const { id } = parseParams(request, idParamSchema);
    ...
    if (inUse > 0) {
      ...
      await audit(request, { action: 'plan.deactivated', entityType: 'SubscriptionPlan', entityId: id });
      return { success: true, deactivated: true, activeSubscriptions: inUse };
    }
    await prisma.subscriptionPlan.delete({ where: { id } });
    await cacheInvalidatePattern('plans:*');
    await audit(request, { action: 'plan.deleted', entityType: 'SubscriptionPlan', entityId: id });
  ...
  app.post('/fix-odds-plans', { preHandler: [app.requireCapability('plans:write')] }, async (request, reply) => {
    ...
    await audit(request, { action: 'fixodds.created', entityType: 'FixOddsPlan', entityId: plan.id, after: input });
    reply.status(201);
    return serializeFixOddsPlan(plan);
  });

  app.patch('/fix-odds-plans/:id', { preHandler: [app.requireCapability('plans:write')] }, async (request) => {
    const { id } = parseParams(request, idParamSchema);
    ...
    await audit(request, { action: 'fixodds.updated', entityType: 'FixOddsPlan', entityId: id, after: input });
    return serializeFixOddsPlan(plan);
  });

  app.delete('/fix-odds-plans/:id', { preHandler: [app.requireCapability('plans:write')] }, async (request) => {
    const { id } = parseParams(request, idParamSchema);
    await prisma.fixOddsPlan.update({ where: { id }, data: { isActive: false } });
    await audit(request, { action: 'fixodds.deactivated', entityType: 'FixOddsPlan', entityId: id });
    return { success: true };
  });
  ...
  app.post('/coupons', { preHandler: [app.requireCapability('coupons:write')] }, async (request, reply) => {
    ...
    await audit(request, { action: 'coupon.created', entityType: 'Coupon', entityId: coupon.id, after: input });
    reply.status(201);
    return coupon;
  });

  app.patch('/coupons/:id', { preHandler: [app.requireCapability('coupons:write')] }, async (request) => {
    const { id } = parseParams(request, idParamSchema);
    ...
    await audit(request, { action: 'coupon.updated', entityType: 'Coupon', entityId: id, after: input });
    return coupon;
  });

  app.delete('/coupons/:id', { preHandler: [app.requireCapability('coupons:write')] }, async (request) => {
    const { id } = parseParams(request, idParamSchema);
  ...
  app.get('/coupons/:id/redemptions', { preHandler: [app.requireCapability('coupons:write')] }, async (request) => {
    const { id } = parseParams(request, idParamSchema);
  ...
  app.post('/promotions', { preHandler: [app.requireCapability('promotions:write')] }, async (request, reply) => {
    ...
    await audit(request, { action: 'promotion.created', entityType: 'Promotion', entityId: promotion.id, after: input });
    reply.status(201);
    return serializePromotion(promotion);
  });

  app.patch('/promotions/:id', { preHandler: [app.requireCapability('promotions:write')] }, async (request) => {
    const { id } = parseParams(request, idParamSchema);
    ...
    await cacheInvalidatePattern('promotions:*');
    await audit(request, { action: 'promotion.updated', entityType: 'Promotion', entityId: id, after: input });
    return serializePromotion(promotion);
  });

  app.delete('/promotions/:id', { preHandler: [app.requireCapability('promotions:write')] }, async (request) => {
    const { id } = parseParams(request, idParamSchema);
  ...
  app.get('/payments', { preHandler: [app.requireCapability('payments:read')] }, async (request) => {
    const query = parseQuery(request, paginationSchema);
    const { skip, take } = skipTake(query);
  ...
  app.get('/subscriptions', { preHandler: [app.requireCapability('payments:read')] }, async (request) => {
    const query = parseQuery(request, paginationSchema);
    const { skip, take } = skipTake(query);
  ...
  app.get('/webhooks', { preHandler: [app.requireCapability('payments:read')] }, async (request) => {
    const query = parseQuery(request, paginationSchema);
    const { skip, take } = skipTake(query);
